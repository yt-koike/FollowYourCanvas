...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    lmm_model = AutoModelForCausalLM.from_pretrained(lmm_path, device_map="cuda", trust_remote_code=True, torch_dtype=torch.float32).eval()
    ...
    re = 0
    for video_name in video_paths[start_index:]:
        ...
        # video_batch_val = val_dataset[start_index + index]
        video_batch_val = {}
        video_batch_val['videoid'] = video_name
        video_batch_val["fps"] = 8
        video_reader = VideoReader(os.path.join(video_dir, video_name))
        framelst = [i for i in range(len(video_reader))] if len(video_reader)< 64 else [i for i in range(64)]
        if len(framelst) < 64:
            last = framelst[-1]
            framelst.extend([last for _ in range(64-len(framelst))])
        frames = video_reader.get_batch(framelst)
        video_batch_val["pixel_values"] = pixel_transforms(torch.from_numpy(frames.asnumpy().transpose(0,3,1,2)) / 255.)
        video_batch_val['video_length'] = len(video_reader)
        ...
        for outpainting_round in range(len(canvas_sizes)):
            ...
            if videos_overlap is not None:
                ...
                for video_overlap in videos_overlap:
                    sub_samples.append(video_overlap.cpu()[0].unsqueeze(0))
                sample_num = len(sub_samples)
                sub_samples = torch.concat(sub_samples)
                save_path = f"{output_dir}/overlaps{video_name}-fps-{fps_tensor[0]}-{prompts[:10].replace('/', ' ')}-{n_frames}f-{outpainting_round}{re}.gif"
                save_dir = os.path.dirname(save_path)
                try:
                    os.makedirs(save_dir, exist_ok=True)
                    save_videos_grid(sub_samples, save_path[:-4] + '.mp4', n_rows=sample_num, n_frames=video_length)
